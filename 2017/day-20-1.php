...
foreach ($lines as $line) {
    preg_match('/p=<(?P<p>[\s\-0-9,]+)>, v=<(?P<v>[\s\-0-9,]+)>, a=<(?P<a>[\s\-0-9,]+)>/', $line, $matches);

    $p = explode(',', $matches['p']);
    $v = explode(',', $matches['v']);
    $a = explode(',', $matches['a']);

    array_walk($p, function (&$item) { $item = trim($item); });
    array_walk($v, function (&$item) { $item = trim($item); });
    array_walk($a, function (&$item) { $item = trim($item); });
...
do {
    for ($j = 0, $count = count($particles); $j < $count; $j++) {
        ...
        $prev = $distances[$j] ?? 0;
        $distances[$j] = max($distances[$j] ?? 0, abs($particles[$j]['p'][0]) + abs($particles[$j]['p'][1]) + abs($particles[$j]['p'][2]));
        $trends[$j] = $distances[$j] - $prev;
    }

    $i++;
    $min = min($trends);
} while (array_count_values($trends)[$min] > 1 || $i < 1000);

print array_search($min, $trends) . PHP_EOL;
