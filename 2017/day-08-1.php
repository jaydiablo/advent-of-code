...
foreach ($lines as $line)  {
    preg_match(
        '/^(?P<register>[a-z]+) (?P<change>inc|dec) (?P<value>[0-9\-]+) if (?P<condition_register>[a-z]+) (?P<sign>[!><=]+) (?P<condition_value>[0-9\-]+)$/',
        $line,
        $matches
    );
    ...
    $changeValue = $matches['value'];
    $conditionRegister = $matches['condition_register'];
    $conditionSign = $matches['sign'];
    $conditionValue = $matches['condition_value'];

    if (!isset($registers[$register])) {
        $registers[$register] = 0;
    }
    if (!isset($registers[$conditionRegister])) {
