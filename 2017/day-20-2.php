...
do {
    ...
    foreach ($particles as $j => $particle) {
    ...
    foreach ($particles as $j => $particle) {
        $position = implode(',', $particles[$j]['p']);

        if (isset($positions[$position])) {
            unset($particles[$j]);
            unset($particles[$positions[$position]]);
    ...
    $i++;
} while (count($particles) > 1 && ($prevCollisions != $collisions || $i < 1000));

print count($particles) . PHP_EOL;
