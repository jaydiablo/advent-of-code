...
foreach ($lines as $line) {
    preg_match('|^(?P<name>[a-z]+) \((?P<weight>[0-9]+)\)(?: -> (?P<subs>[a-z\s,]+))?$|', $line, $matches);

    $name = $matches['name'];
    $weight = $matches['weight'];

    if (isset($matches['subs'])) {
        $subs = explode(', ', trim($matches['subs']));
        array_walk($subs, function (&$value) {
            $value = ['name' => $value, 'children' => []];
        });
    } else {
        $subs = [];
    }

    $programs[] = ['name' => $name, 'children' => $subs, 'weight' => $weight];
}

usort($programs, function ($a, $b) {
    return (int) (count($a['children']) > count($b['children']));
});

function moveProgram($subject, &$children) {
    $found = false;
    $i = 0;
    foreach ($children as $child) {
        if ($child['name'] === $subject['name']) {
            $children[$i]['children'] = $subject['children'];
            $children[$i]['weight'] = $subject['weight'];

            return true;
        } elseif (!empty($child['children'])) {
            $found = moveProgram($subject, $children[$i]['children']);
            if ($found === true) {
                return true;
            }
        }
...
while (count($programs) > 1) {
    $toCheck = array_shift($programs);

    if (moveProgram($toCheck, $programs) !== true) {
        array_push($programs, $toCheck);
    }
}

function isBalanced($input) {
    $sums = [];
    foreach ($input['children'] as $program) {
        $sums[] = $program['weight'] + isBalanced($program);
    }

    $sumCounts = array_count_values($sums);

    if (count($sumCounts) > 1) {
        arsort($sumCounts);

        $sumCounts = array_keys($sumCounts);

        $expected = array_shift($sumCounts);
        $actual = array_shift($sumCounts);

        $position = array_search($actual, $sums);

        print $input['children'][$position]['weight'] + ($expected - $actual) . PHP_EOL;
        exit;
    }
