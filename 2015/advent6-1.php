...
foreach ($instructions as $line) {
    ...
    list($left, $right) = explode(' through ', $line);
    ...
    for ($x = $left[0]; $x <= $max_x; $x++) {
        for ($y = $left[1]; $y <= $max_y; $y++) {
            if ($type == 'toggle') {
                $matrix[$x . 'x' . $y] = !($matrix[$x . 'x' . $y]);
                //print "toggling $x x $y" . PHP_EOL;
            } elseif ($type == 'turn on') {
                $matrix[$x . 'x' . $y] = true;
                //print "turning on $x x $y" . PHP_EOL;
            } else {
                $matrix[$x . 'x' . $y] = false;
                //print "turning off $x x $y" . PHP_EOL;
            }
        }
    }
}
...
foreach ($matrix as $position => $status) {
    if ($status === true) {
        $on++;
    }
}

print count($matrix) . PHP_EOL;
print $on . PHP_EOL;
