...
foreach ($lines as $line) {
    ...
    $tracking[$deer] = ['traveled' => 0, 'restDuration' => 0, 'moveDuration' => 0, 'resting' => false, 'totalRest' => 0, 'totalMove' => 0, 'points' => 0];
}
...
//    print $i . PHP_EOL;
    ...
    // Have to determine who is in the lead and award a point
    $max = 0;
    $winners = [];
    foreach ($tracking as $deer => $data) {
        if ($data['traveled'] > $max) {
            $winners = [$deer];
            $max = $data['traveled'];
        } elseif ($data['traveled'] == $max) {
            $winners[] = $deer;
        }
    }

    foreach ($winners as $winner) {
        $tracking[$winner]['points']++;
    }
}
